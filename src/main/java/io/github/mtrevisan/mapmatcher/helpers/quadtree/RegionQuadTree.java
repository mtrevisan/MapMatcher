 ...
 */
public class RegionQuadTree<R extends Region> implements RegionTree<R>{
	...
	private final R envelope;
	private final RegionQuadTree<R>[] children;
	private final List<R> regions;

	private final int maxRegionsPerNode;


	public static <R extends Region> RegionQuadTree<R> create(final R envelope, final int maxRegionsPerNode){
		return new RegionQuadTree<>(envelope, maxRegionsPerNode);
	}


	@SuppressWarnings("unchecked")
	private RegionQuadTree(final R envelope, final int maxRegionsPerNode){
		...
		this.children = (RegionQuadTree<R>[])Array.newInstance(RegionQuadTree.class, 4);
...
//	@Override
//	public void insert(final R region){
//		insert(region, "");
//	}
//
//	private void insert(final R region, final String code){
//		if(children[0] != null){
...
//	}
	...
	@Override
	public void insert(final R region){
		final Stack<InsertItem<R>> stack = new Stack<>();
		stack.push(new InsertItem<>(this, "", region));
		while(!stack.isEmpty()){
			final InsertItem<R> item = stack.pop();
			final RegionQuadTree<R> itemNode = item.node;
			final String itemCode = item.code;
			final R itemRegion = item.region;
			...
			if(itemNode.regions.size() > itemNode.maxRegionsPerNode){
				...
				while(i < itemNode.regions.size()){
					final R nodeRegion = itemNode.regions.get(i);
	...
	private static class InsertItem<R extends Region>{
		final RegionQuadTree<R> node;
		final String code;
		final R region;

		private InsertItem(final RegionQuadTree<R> node, final String code, final R region){
			this.node = node;
			this.code = code;
			this.region = region;
		}
	...
	protected int getChildIndex(final R region){
...
//	@Override
//	public boolean delete(final R region){
...
//	}
	...
	@Override
	public boolean contains(final R region){
		final Stack<RegionQuadTree<R>> stack = new Stack<>();
		stack.push(this);
		while(!stack.isEmpty()){
			final RegionQuadTree<R> node = stack.pop();

			final int index = node.getChildIndex(region);
			if(index == INDEX_SELF){
				if(node.children[0] != null)
					for(final RegionQuadTree<R> child : node.children)
			...
			for(final R nodeRegion : node.regions)
	...
	@Override
	public Collection<R> query(final R region){
