...
class BidirectionalPathSummaryTest{
	...
	void should_return_path_consisting_of_vertices(){
		Node first = new Node(new Coordinate(1., 2.));
		Node second = new Node(new Coordinate(2., 2.));
		List<Edge> path = new ArrayList<>(List.of(new Edge(first, second,
	...
	void should_return_path_distance(){
		...
		ArrayList<Edge> path = new ArrayList<>(List.of(new Edge(first, second,
	...
	void should_return_path_search_boundaries(){
		...
		BidirectionalPathSummary pathSummary = new BidirectionalPathSummary(new ArrayList<>(),
			...
			new HashSet<>(Arrays.asList(thirdNode, firstNode, secondNode)));

		Collection<List<Node>> result = pathSummary.searchBoundaries();

		Assertions.assertEquals(2, result.size());
		Assertions.assertEquals(Arrays.asList(secondNode, firstNode, thirdNode), result.toArray()[0]);
		Assertions.assertEquals(Arrays.asList(secondNode, firstNode, thirdNode), result.toArray()[1]);
	}
	...
	void should_return_whether_the_path_is_empty(){
		...
		Node node1 = new Node(new Coordinate(14.552797, 121.058805));
		Node node2 = new Node(new Coordinate(14.593999, 120.994260));
		path = new ArrayList<>(List.of(new Edge(node1, node2, FACTORY.createLineString(new Coordinate[]{node1.getCoordinate(), node2.getCoordinate()}))));
